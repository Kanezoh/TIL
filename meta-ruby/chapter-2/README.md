...
モジュールは継承チェーンではインクルードしたクラスの真上に追加される。複数のモジュールをインクルードする際には
オーバーライドなどの問題があるので順番に気をつける。  
~~~
class C
  include ModA
  include ModB
...
c = C.new
c.ancestors => [:C,:ModB,:ModA,:Object,:Kernel,:BasicObject] # ModAが押し上げられている  
~~~  

## 2.6 まとめ

- オブジェクト：インスタンス変数とクラスへの参照で構成される
- メソッドはクラスに住んでる  
- Classはnewやsuperclassでインスタンスの生成、継承ができるようにしたModule
- 継承チェーンの動きは右、上
- モジュールはクラスの真上にインクルードされる(prependを使えば真下)
- メソッドを呼び出すとレシーバがselfになる
- Refinementsでモンキーパッチの副作用を抑えられる
